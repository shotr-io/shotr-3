...
{
    public class DpiScaledCheckbox : MetroCheckBox
    {
        private Size _nsize { get; set; }
        private Point _nlocation { get; set; }

        public DpiScaledCheckbox() : base()
        {
            if (LicenseManager.UsageMode != LicenseUsageMode.Designtime)
            {
                ...
                EventHandler handler = (sender, args) =>
                {
                    ...
                    Console.WriteLine("DPI Scaled Checkbox: {0} - {1}x{2} ({3}x{4})", Text, Size.Width, Size.Height, _nsize.Width, _nsize.Height);
                };
