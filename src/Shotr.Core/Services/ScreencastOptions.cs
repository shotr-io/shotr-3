...
namespace Shotr.Core.Services
{
    ...
    public class FFmpegHelperService : ExternalCliManager
    {
        public static readonly int Libmp3LameQscaleEnd = 9;
        ...
        public FFmpegHelperService(ScreencastOptions options)
        {
        ...
        {
            var devices = new DirectShowDevices();

            if (File.Exists(Options.FFmpeg.CLIPath))
            {
                var arg = "-list_devices true -f dshow -i dummy";
    ...
    public class FFmpegOptions
    {
        ...
        public bool IsVideoSourceSelected => !string.IsNullOrEmpty(VideoSource) && !VideoSource.Equals(FFmpegHelperService.SourceNone, StringComparison.InvariantCultureIgnoreCase);

        public bool IsAudioSourceSelected => !string.IsNullOrEmpty(AudioSource) && !AudioSource.Equals(FFmpegHelperService.SourceNone, StringComparison.InvariantCultureIgnoreCase);

        public FFmpegOptions()
        {
            // General
            VideoSource = FFmpegHelperService.SourceGdiGrab;
            AudioSource = FFmpegHelperService.SourceNone;
    ...
    public class ScreencastOptions
    {
        ...
        public string GetFFmpegArgs(bool isCustom = false)
        {
            ...
            var fps = isCustom ? "$fps$" : ScreenRecordFPS.ToString();
           
            if (FFmpeg.IsVideoSourceSelected)
            {
                if (FFmpeg.VideoSource.Equals(FFmpegHelperService.SourceGdiGrab, StringComparison.InvariantCultureIgnoreCase))
                {
