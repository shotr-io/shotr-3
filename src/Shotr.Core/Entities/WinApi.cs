    ...
    public class WinApi
    {
        ...
        public static Bitmap CaptureCursor(ref int x, ref int y)
        {
            try
            {
                Bitmap bmp;
                IntPtr hicon;
                var ci = new WinApi.CURSORINFO();
                WinApi.ICONINFO icInfo;
                ci.cbSize = Marshal.SizeOf(ci);
                if (WinApi.GetCursorInfo(out ci))
                {
                    if (ci.flags == WinApi.CURSOR_SHOWING)
                    {
                        hicon = WinApi.CopyIcon(ci.hCursor);
                        if (WinApi.GetIconInfo(hicon, out icInfo))
                        {
